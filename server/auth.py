    ...
    if current_user.admin:
        if request.method == 'POST':
            ...
            if user:
                ...
                new_user = User(email=email, name=name, hashed_password=generate_password_hash(password, method='sha256'))
    ...
    if current_user.admin:
        ...
        if request.method == 'POST':
            ...
            if len(email) < 4:
                ...
                user.admin = admin
                if password:
                    user.hashed_password = generate_password_hash(password, method='sha256')
                db.session.add(user)
                db.session.commit()
